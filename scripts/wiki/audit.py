"""Deterministic wiki audit harness for dev.news-wiki."""
...
def _link_resolver() -> WikiLinkResolver:
...
def extract_wiki_links(content: str) -> list[str]:
    return _link_resolver().extract_links(content)


def resolve_wiki_link(target: str, source_file: Path) -> tuple[bool, str]:
    return _link_resolver().resolve(target, source_file)


def scan_broken_links() -> list[BrokenLink]:
    broken: list[BrokenLink] = []
    resolver = _link_resolver()
    for path in iter_wiki_markdown_files():
        content = path.read_text(encoding="utf-8")
        source_rel = str(path.relative_to(ROOT)).replace("\\", "/")
        for target in resolver.extract_links(content):
            ok, reason = resolver.resolve(target, path)
            if not ok:
                broken.append(
                    BrokenLink(
...
def scan_index_gaps() -> list[IndexGap]:
    ...
    for index_path in index_files:
        ...
        for target in resolver.extract_links(content):
            ok, reason = resolver.resolve(target, index_path)
            if not ok:
                gaps.append(
                    IndexGap(
...
def _format_deterministic_context(findings: DeterministicFindings) -> str:
    ...
    if findings.broken_links:
        ...
        for item in findings.broken_links[:50]:
            parts.append(f"- {item.source_path}: {item.link} -> {item.reason} (target={item.target})")

    if findings.index_gaps:
        parts.append("\n### Harness index gaps")
        for item in findings.index_gaps[:50]:
            parts.append(f"- {item.index_path}: {item.link} -> {item.reason} (target={item.target})")
...
def _validate_evidence(evidence: dict[str, Any], finding_index: int, evidence_index: int) -> None:
    ...
    if not note:
        raise ValueError(f"findings[{finding_index}].evidence[{evidence_index}].note is required.")
...
def validate_audit_response(response: dict[str, Any]) -> None:
    ...
    output = require_mapping(response, "output", label="Audit field 'output'")
    ...
    if findings:
        ...
        for finding in findings:
            ...
            for evidence in finding.get("evidence", []) or []:
                parts.append(render_citation_line(evidence))
    ...
    if deterministic:
        ...
        if deterministic.broken_links:
            ...
            for item in deterministic.broken_links:
                parts.append(f"- {item.source_path}: {item.link} — {item.reason} (`{item.target}`)")
            parts.append("")
...
def save_audit_output(
    ...
    *,
    ...
    slug = sanitize_slug(str(output.get("filename_slug", "")).strip(), fallback="wiki-audit")
    return save_dated_markdown(
        OUTPUTS_DIR,
        slug=slug,
        body=render_audit_markdown(response, deterministic=deterministic),
        dry_run=dry_run,
    )
    ...
    *,
    ...
    try:
        ...
        if not dry_run and resolved.get("output", {}).get("should_save"):
            saved_path = save_audit_output(resolved, dry_run=False, deterministic=resolved_deterministic)
            if saved_path:
                result.saved = True
                result.output_path = str(saved_path.relative_to(ROOT))
        elif dry_run:
            planned = save_audit_output(resolved, dry_run=True, deterministic=resolved_deterministic)
            if planned:
                result.output_path = str(planned.relative_to(ROOT))

        if not result.output_path and resolved.get("output", {}).get("should_save"):
            slug = sanitize_slug(str(resolved.get("output", {}).get("filename_slug", "")).strip(), fallback="wiki-audit")
...
def main() -> int:
    ...
    if reject_fixture_provider(args.provider, script="audit_wiki"):
        return 1
